...
// GET - Webhook verification (Modificado para la Universidad)
export async function GET(request: Request) {
  try {
    ...
    // Si Meta nos manda el token correcto, le respondemos con el challenge al instante
    if (verifyToken === 'mi_universidad_secret_2026' && challenge) {
    ...
    // Si es cualquier otra llamada, devolvemos un texto plano con el challenge por seguridad
    if (challenge) {
      return new Response(challenge, {
        status: 200,
        headers: { 'Content-Type': 'text/plain' },
